...
The [artifact-contract index](docs/contracts/README.md) includes eight accepted, binding subordinate artifact-specific contracts: Project Charter, Workstream, Task Contract, Context Packet, Execution Result, Evidence Bundle, Review Record, and Decision Record. It also includes State Snapshot as the ninth **Proposed**, documentation-only artifact-specific contract. None introduces an executable schema, template, validator, state engine, synchronization engine, workflow, runtime, or product functionality. No other canonical artifact contract remains listed as future work once this candidate is present; that status does not authorize a follow-on phase. CNTX remains a public core that is model-, vendor-, runtime-, and domain-agnostic and remains independent of private reference implementations.
