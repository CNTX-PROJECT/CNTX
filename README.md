...
The architecture index also exposes the **Proposed** [CNTX Schema Resource
...
under issue #72 and attributable EIGENAAR authority comment `5221792750`. The
documentation-only candidate proposes exact Schema Identifier/Version keys, a
...
handling, determinism, provenance, and security/privacy boundaries. It changes
no Accepted contract, architecture, schema, test, identity, version, or Core
Artifact JSON Binding and creates no executable catalog, resolver, registry,
cache, bundler, validator, validation output, conformance tool,
