...
The architecture index now also exposes the **Proposed and inactive** [Context
...
the Proposed [Context Packet Schema Version `1.0.0`](schemas/context-packet/1.0.0/schema.json),
...
The candidate provides no automatic selection, retrieval, ranking, RAG,
disclosure, transformation, prompt, workflow, or runtime behavior. Validation,
Draft PR publication, or review cannot activate the schema or authorize an
Execution Result schema; separate exact-head Owner / Final Authority acceptance
is required before promotion and integration.
