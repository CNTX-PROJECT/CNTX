...
The architecture index also lists a **Proposed**, documentation-only [Common Artifact Envelope schema-boundary candidate](docs/architecture/common-artifact-envelope-schema-boundary.md) with [ADR-0004](docs/architecture/adr/0004-common-artifact-envelope-schema-boundary.md). The candidate classifies shared metadata ownership before any executable schema decision. It is not accepted architecture and authorizes no concrete fields, serialization, validator, Layer 5 mechanism, runtime, or integration.
