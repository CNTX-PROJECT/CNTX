# CNTX Common Artifact Envelope Schema Boundary (ARCH-004 candidate)

## Status and authority

Status: **Proposed**.

This document is an independently reviewable, documentation-only architecture candidate prepared under issue #32. It is not an accepted repository decision and does not alter [ARCH-001](core-contract.md), [ARCH-002](contract-identity-versioning.md), [ARCH-003](artifact-contract-schema-architecture.md), any accepted artifact contract, or final human authority. It becomes binding only after the review and attributable Owner / Final Authority acceptance required by repository governance.

The candidate defines semantic ownership boundaries for a future Common Artifact Envelope. It does not define concrete fields, names, requiredness, types, nesting, cardinality, identifiers, timestamps, digests, schema language or dialect, serialization, packaging, publication, validation, or runtime behavior.
...
This candidate answers only those boundary questions. Classification here assigns semantic ownership; it does not decide whether or how a concept is represented in any future schema.
...
| Issues #30, #31, and #32 | Schema work remains core-first and documentation-only; Layer 5 and executable or serialization decisions remain deferred; issue #32 is the bounded authority for this candidate. |
...
Transport, storage, provider, domain, private-implementation, execution, orchestration, and product concerns are also outside this candidate. Their exclusion does not assign them to a later layer or authorize later work.
...
No decision about schema language or dialect, executable envelope structure, schema identity or version assignment, packaging, publication, or validation is authorized by this candidate. Before any such decision is proposed:

1. this boundary candidate must receive the required architectural review and separate attributable Owner / Final Authority acceptance;
2. the exact accepted revision and its governing ARCH-001 through ARCH-003 baseline must be identified;
3. the later task must define an explicit allowlist and preserve the four ownership categories and nine contract boundaries;
4. schema identity/versioning and language/dialect choices must be presented as separate, reviewable decisions rather than embedded assumptions;
5. security and privacy analysis must confirm that public definitions cannot require secrets, credentials, personal data, production configuration, or private implementation context; and
6. the later task must retain the distinction between schema validity, contract conformance, review, approval, integration, and human authority.
...
This candidate contains no private project context and establishes no discovery, registry, negotiation, retrieval, transport, storage, access-control, or runtime mechanism. Security or privacy questions that require such mechanics need a separate private assessment and an explicitly authorized public decision boundary.

## Review and acceptance gate

Lead Architect review must verify traceability to ARCH-001 through ARCH-003, the four-way boundary matrix, all nine contract cross-checks, non-authority semantics, public/private separation, and the continued deferral of Layer 5 and executable mechanics.

Only an attributable Owner / Final Authority decision on the exact reviewed revision can change this candidate from Proposed to Accepted. Review completion, a passing check, a draft pull request, mergeability, or the existence of this document is not acceptance and does not authorize integration or merge.
