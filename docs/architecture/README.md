...
CONTRACT-009, the [State Snapshot artifact contract](../contracts/state-snapshot-contract.md), is **Proposed**, documentation-only, and not binding or accepted unless and until an applicable Owner-authorized acceptance records that state. It is subordinate to accepted ARCH-001, ARCH-002, ARCH-003, and accepted CONTRACT-001 through CONTRACT-008; it specializes State Snapshot semantics only and does not alter accepted architecture, any accepted artifact contract, or final human authority. CONTRACT-001 through CONTRACT-008 remain **Accepted**.
