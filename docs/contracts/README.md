...
The accepted architecture contracts in [the architecture directory](../architecture/README.md) are the higher normative basis for this directory. Artifact-specific contracts specialize accepted canonical artifacts without redefining their primary meanings, classifications, authority model, lifecycle, identity and versioning semantics, provenance, or public/private boundaries. CONTRACT-001 through CONTRACT-008 are **Accepted**, binding subordinate specializations in their canonical order. CONTRACT-009, the State Snapshot contract, is the ninth **Proposed**, documentation-only specialization, governed by accepted ARCH-001, ARCH-002, ARCH-003, and accepted CONTRACT-001 through CONTRACT-008. Its proposed status is neither binding nor an authority source and it does not change final human authority.
...
The following order follows the accepted dependency direction. Project Charter, Workstream, Task Contract, Context Packet, Execution Result, Evidence Bundle, Review Record, and Decision Record are the first eight accepted, binding subordinate contracts. [State Snapshot](state-snapshot-contract.md) is the ninth **Proposed** contract and specializes derived orientation and handoff semantics only. All nine contracts are either Accepted or Proposed; no canonical artifact contract remains listed as future work. The canonical order and proposed status do not authorize an executable schema, template, payload, validator, registry, selector, engine, workflow, runtime, or implementation.
...
| 9 | [State Snapshot](state-snapshot-contract.md) | **Proposed** — CONTRACT-009 |
