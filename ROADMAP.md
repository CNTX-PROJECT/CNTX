...
Proposed for review; the capability is not Accepted, active, implemented, or
authorized for use. Every later step remains separately governed.
...
| 4 | Source, provenance and freshness controls | First Definition candidate Proposed; capability not active | Make source identity, revision, time, uncertainty, and unverifiable conditions more explicit |
...
are **Proposed** under issue #128 and attributable EIGENAAR / Final Authority
issue-contract acceptance comment `5251980826`. The documentation-only
candidate proposes exactly one Epistemic Provenance and Freshness Extension
Module Definition with local name `epistemic-provenance-freshness`, proposed
Definition Identifier
`https://github.com/CNTX-PROJECT/CNTX/extension-module-definitions/epistemic-provenance-freshness`,
and proposed initial Definition Version `1.0.0`.
...
allocate or activate nothing. No Profile, representation, schema, rule, tool,
implementation, execution, evidence, support, release, hosting, or deployment
is created or authorized.

## Detailed project status and roadmap

CNTX has completed its initial Public-Core specification and prerelease cycle within the Accepted ARCH-027 completion and maintenance boundary. The current public baseline additionally includes the [Accepted architecture through ARCH-037 and one Proposed ARCH-038 candidate](docs/architecture/README.md). The repository provides public governance, thirty-seven Accepted architecture sources and ADRs plus one Proposed architecture source and ADR, nine Accepted artifact contracts, ten Accepted Schema Versions 1.0.0 with synthetic cases, one integrated minimal validation and integrity Tool/Implementation slice, and one immutable unsupported prerelease, 0.1.0-prealpha.1. CNTX remains model-, vendor-, runtime-, and domain-agnostic. Neither the integrated slice nor the Proposed candidate creates a supported release line, support service, certification, hosting, deployment, workflow, CI, product, or final-human authority.
...
The architecture index now also exposes the **Proposed** [CNTX Epistemic
...
acceptance comment `5251980826`. The documentation-only candidate proposes one
exact Extension Module Definition Identifier and initial Version `1.0.0`, six
closed logical source categories, exact source identity/revision and
provenance responsibilities, four separate temporal coordinates, explicit
...
human authority. Proposed status, repository presence, validation, review, and
mergeability do not allocate or activate the Identifier or Version. No Profile,
representation, property, schema, Schema Resource, testcase, rule, policy,
Tool/Implementation Version, dependency, code, execution, evidence instance,
release, publication, support, certification, hosting, deployment, merge
permission, or follow-on authority is created.
